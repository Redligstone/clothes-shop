  ...
  const formik = useFormik({
    ...
    onSubmit: (values) => {
      ...
      const order = {
        ...
        товары: items,
        цена: totalPrice,
        оплата: values.payment ? "Наличными" : "",
      };
  ...
  const handleChange = (event) => {
    const value =
      event.target.type === "checkbox"
        ? (event.target.payment = true)
        : event.target.payment;
    formik.setFieldValue(event.target.payment, value);
  };
      ...
      <div className={s.container}>
        ...
        <form onSubmit={formik.handleSubmit}>
          <section className={s.main}>
            ...
            <div className={s.error}>{formik.errors.payment}</div>
          )}
